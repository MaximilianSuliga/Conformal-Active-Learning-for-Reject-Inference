...
def random_al(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_):
    ...
    # Randomly sample rejected applications of sample size of 20% of accepted applications and turn their prediction
    # from BAD to GOOD
    mask = test_['BAD_pred'] == 1
    sample_size = int(0.2 * len(test_[test_['BAD_pred'] ==0]))
...
def icp(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_, error_func):
    ...
    # Sample the least confident rejected applications of sample size of 20% of accepted applications and turn their prediction
    # from BAD to GOOD
    sample_size = int(0.2 * len(test_[test_['BAD_pred'] ==0]))
    sample_size=min(sample_size, len(test_[test_['BAD_pred'] ==1]))
...
def no_al_ri(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_):
    '''Benchmark strategy of No Active Learning'''
    ...
    bads_pred = test_[test_['BAD_pred']==1].drop(columns='BAD_pred')
    bads_pred['BAD'] = 1
    
    new_data = pd.concat([start, goods_pred, bads_pred])

    return new_data, metrics

def random_al_ri(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_):
    '''Active Learning strategy of selecting a random sample of AL instances'''
...
def icp_ri(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_, error_func):
    '''Active Learning strategy of using an Inductive Conformal Predictor for assessing Confidence of predicted BADs.
    Reqires and error function of the class type to be put in for the error_func parameter.'''
    ...
    bads_pred = test_[test_['BAD_pred']==1].drop(columns=['BAD_pred', 'pred_conf'])
    bads_pred['BAD'] = 1
...
def icp_prob_ri (start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_):
    '''Active Learning strategy of using an Inductive Conformal Predictor for assessing Confidence of predicted BADs.
    Inverse Probability Error Function used as Error Function'''
    
    return icp_ri(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_, InverseProbabilityErrFunc())
 
def icp_nnmargin_ri(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_):
    '''Active Learning strategy of using an Inductive Conformal Predictor for assessing Confidence of predicted BADs.
    Nearest Neighbour Margin Error Function used as Error Function''' 
    return icp_ri(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_, NearestNeighbourMargin())
