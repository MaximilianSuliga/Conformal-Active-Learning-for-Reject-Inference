...
def random_al(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_):
    ...
    # Randomly sample rejected applications of sample size of the threshold of accepted applications and turn their prediction
    # from BAD to GOOD
    mask = test_['BAD_pred'] == 1
    sample_size = int(threshold_ * len(test_[test_['BAD_pred'] ==0]))
...
def icp(start, start_trans, test, test_trans_, threshold_, cost_matrix_, cost_matrix_test_, error_func):
    ...
    # Sample the least confident rejected applications of sample size of the threshold of accepted applications and turn their prediction
    # from BAD to GOOD
    sample_size = int(np.mean(threshold_ * len(test_[test_['BAD_pred'] ==0])))
    sample_size=min(sample_size, len(test_[test_['BAD_pred'] ==1]))
